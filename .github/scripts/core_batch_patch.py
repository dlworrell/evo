...
path = Path('src/population_evaluation.c')
text = path.read_text()
marker = 'evo_status_t evo_population_evaluate_ready_with_worker_backend(\n'
if 'evo_population_evaluate_ready_with_batch_evaluator(' not in text:
    if marker not in text:
        raise SystemExit('population evaluator insertion marker missing')
    block = r'''static bool evo_batch_fitness_is_zero(const evo_fitness_t *fitness)
{
...
wrapper_marker = 'evo_status_t evo_population_restore_evaluations_allocate(\n'
if 'evo_population_evaluate_with_batch_evaluator(' not in text:
    if wrapper_marker not in text:
        raise SystemExit('initial batch wrapper marker missing')
    wrapper = r'''evo_status_t evo_population_evaluate_with_batch_evaluator(
    const evo_problem_t *problem,
    const evo_config_t *config,
    void *context,
    uint64_t generation,
